...
class JointAutoencoder(torch.nn.Module):
        ...
        for i in range(x.size(0)):
            for j in range(x.size(0)):
                if labels[i] == labels[j]:
                    weight_matrix[i][j] = np.exp(-np.linalg.norm(x[i] - x[j])**2)/len(x == labels[i])  
                else:
                    weight_matrix[i][j] = np.exp(-np.linalg.norm(x[i] - x[j])**2)/len(x == labels[i])
    ...
    def _get_loss(self, x):
        ...
        print(reconstruction_loss, 0.5000*cluster_loss)
        return reconstruction_loss + 0.5000*cluster_loss
